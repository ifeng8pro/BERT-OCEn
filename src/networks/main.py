...
def build_network(net_name, dataset,embedding_size=None, pretrained_model=None, update_embedding=True,
                  ...
                  word_vectors_cache='../data/word_vectors_cache', attention_size=100, n_attention_heads=1,finetune=0):
    ...
    if pretrained_model is not None:
        ...
        if pretrained_model in ['bert']:
            if str(dataset)in ['Yixing_Dataset', 'Nanchang_Dataset']:
                embedding = BERT(pretrained_model_name='bert-base-chinese', cache_dir='../data/bert_cache/chinese',finetune=finetune)
            else:
                embedding = BERT()
    else:
