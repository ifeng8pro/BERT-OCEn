...
class OCSVM(object):
    ...
    def train(self, dataset: BaseADDataset, device: str = 'cpu', n_jobs_dataloader: int = 0):
        ...
        train_loader, _ = dataset.loaders(batch_size=1, num_workers=n_jobs_dataloader)
        ...
        for data in train_loader:
            _, text, _, weights,_ = data
            text, weights = text.to(device), weights.to(device)
            X_batch = self.embedding(text)  # X_batch.shape = (batch_size, embedding_size)
    ...
    def test(self, dataset: BaseADDataset, device: str = 'cpu', n_jobs_dataloader: int = 0):
        ...
        _, test_loader = dataset.loaders(batch_size=1, num_workers=n_jobs_dataloader)
        ...
        for data in test_loader:
            idx, text, label_batch, weights,my_index = data
            text = text.to(device)
        ...
        # 写出
        df_result = pd.DataFrame(my_indexs, columns=['my_index'])
        df_result['idx'] = idxs
        df_result['labels'] = labels
        df_result['score'] = scores.tolist()
        df_result.to_csv('../result/yixing/10%/ocsvm/yx_temp5.csv', index=False)
        # Compute AUC
