    ...
    def train(self, dataset: BaseADDataset, net: CVDDNet):
        ...
        for epoch in range(self.n_epochs):
            ...
            for data in train_loader:
                _, text_batch, _, _, my_index = data
                train_my_index.extend(list(my_index))
                ...
                loss_emp = torch.mean(1/ torch.mean(scores, dim=1))
        ...
        train_index = train_my_index
        return net,train_index

    def test(self, dataset: BaseADDataset, net: CVDDNet, ad_score='context_dist_mean',normal_class=None):
        logger = logging.getLogger()
        ...
        with torch.no_grad():
            for data in test_loader:
                idx, text_batch, label_batch, _, my_index = data
                text_batch = text_batch.to(self.device)
                text_batch = text_batch.to(self.device)
                ...
                my_index_idx_label_score += list(zip(my_index, idx,
                                                 label_batch.cpu().data.numpy().tolist(),
                                                 ad_scores.cpu().data.numpy().tolist()))
        ...
        # Compute AUC
        my_index, idx, labels, scores = zip(*my_index_idx_label_score)
        my_index = np.array(my_index)
        labels = np.array(labels)
        scores = np.array(scores)
        idx = np.array(idx)

        df_result = pd.DataFrame(my_index.tolist(), columns=['my_index'])
        df_result['idx'] = idx.tolist()
        df_result['labels'] = labels.tolist()
        df_result[str(normal_class)] = scores.tolist()
        df_sorted = df_result.sort_values('my_index', ascending=True)
        ...
        score_class = df_sorted[str(normal_class)].values.tolist()
        return score_class
