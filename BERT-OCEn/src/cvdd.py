...
class CVDD(object):
    ...
    def set_network(self, net_name, dataset,normal_class, pretrained_model, embedding_size=None, attention_size=150,
                    n_attention_heads=3, finetune=0):
        ...
        self.net = build_network(net_name, dataset,embedding_size=embedding_size, pretrained_model=pretrained_model,
                                 ...
                                 n_attention_heads=n_attention_heads,finetune=finetune)
    ...
    def train(self, dataset: BaseADDataset, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 25,
        ...
        self.net, train_index = self.trainer.train(dataset, self.net)
    ...
    def test(self, dataset: BaseADDataset, device: str = 'cuda', n_jobs_dataloader: int = 0, normal_class=None):
        """Tests the CVDD model on the test data."""

        if self.trainer is None:
            self.trainer = CVDDTrainer(device, n_jobs_dataloader)

        normal_class_score = self.trainer.test(dataset, self.net, ad_score=self.ad_score, normal_class=normal_class)
