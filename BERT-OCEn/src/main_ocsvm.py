...
################################################################################
@click.command()
@click.argument('dataset_name', type=click.Choice(['reuters', 'newsgroups20', 'imdb','yixing']))
...
def main(dataset_name, xp_path, data_path, load_config, load_model, seed, kernel, nu, tokenizer, clean_txt,
    ...
    #print_text_samples(dataset.test_set, dataset.encoder, idx_normal,export_file=xp_path + '/normals', title='Most normal examples')
    #print_text_samples(dataset.test_set, dataset.encoder, idx_outlier,export_file=xp_path + '/outliers', title='Most anomalous examples')
